...
def download_image(download_image_menu_loc):
    moveTo(download_image_menu_loc.left - 35, download_image_menu_loc.top + 25, duration=0.2)
    click()

    download_menu_loc = find_image('images/ideogram/download-option.png', 0.8)
    moveTo(download_menu_loc.left + 50, download_menu_loc.top + 20, duration=0.2)
    click()
...
def ideogram_download():
    click(find_image('images/tabs/seasoninspired_chrome.png', 0.8), duration=0.2)
