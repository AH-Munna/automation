...
def click_and_write(image_name, text:str):
    image_loc = find_image(get_resource_path(image_name), confidence=0.8)
...
def wordpress_paste_app(post_title:str, meta_description:str, keywords:str, post_content="", also_paste_content=False):
    ...
    """
    browswer_tab_loc = find_image(get_resource_path('images/tabs/new_post_wp.png'), confidence=0.8, tries=2, no_exit=True)
    if browswer_tab_loc is None:
        browswer_tab_loc = find_image(get_resource_path('images/tabs/edit_post_wp.png'), confidence=0.8, tries=2)

    click(browswer_tab_loc, duration=0.5)
    sleep(0.5)

    add_description_loc = find_image(get_resource_path('images/wp/main_title.png'), confidence=0.5)
    ...
    def find_edit_snippet():
        edit_snippet_loc = find_image(get_resource_path('images/wp/edit_snippet.png'), tries=2, confidence=0.8, no_exit=True)
        print(edit_snippet_loc)
        if edit_snippet_loc is None:
            click(find_image(get_resource_path('images/wp/rank_math.png'), confidence=0.8), duration=0.5)
    ...
    # social tab
    sleep(0.5)
    click(find_image(get_resource_path('images/wp/social_tab.png'), confidence=0.8), duration=0.5)
    x, y = position()
    ...
    click(find_image(get_resource_path('images/wp/rank_math_modal_close.png'), confidence=0.8), duration=0.5)

    click_and_write('images/wp/keyword_rank_math.png', keywords)

    click(find_image(get_resource_path('images/wp/main_block.png'), confidence=0.8), duration=0.5)
